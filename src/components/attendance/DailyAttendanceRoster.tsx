...
function StatusCell({ status }: { status: string }) {
  ...
  const c =
    s === 'NA'
      ? { label: '-', className: 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-400 dark:text-slate-500' }
      : s === 'LATE'
        ? { label: 'L', className: 'border-amber-200 bg-amber-50 text-amber-900 dark:border-amber-800 dark:bg-amber-950/50 dark:text-amber-100' }
      : s === 'PRESENT'
      ? { label: 'P', className: 'border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-100' }
      : s === 'LEAVE'
        ? { label: 'L', className: 'border-rose-200 bg-rose-50 text-rose-800 dark:border-rose-800 dark:bg-rose-950/45 dark:text-rose-100' }
        : { label: 'A', className: 'border-slate-200 dark:border-slate-700 bg-slate-100 text-slate-700 dark:text-slate-200' };
  return (
    <span
      className={cn(
...
export function DailyAttendanceRoster({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700/90 bg-white dark:bg-slate-900 shadow-sm shadow-slate-200/40">
      <div className="relative border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-slate-50 via-white to-indigo-50/30 px-5 py-5 dark:from-slate-900 dark:via-slate-950 dark:to-slate-900 sm:px-6">
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="text-base font-bold tracking-tight text-slate-900 dark:text-slate-50">Attendance overview</h3>
              </div>
            </div>
          </div>

          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
            <div className="inline-flex rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/90 p-1 shadow-inner">
              {periodTabs.map((t) => (
                <button
                  key={t.id}
                  type="button"
                  ...
                  className={cn(
                    ...
                    period === t.id
                      ? 'bg-white dark:bg-slate-900 text-indigo-700 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700/80'
            ...
            <div className="flex items-center gap-2 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 py-2 text-xs text-slate-600 dark:text-slate-300 shadow-sm">
              <CalendarDays className="h-4 w-4 shrink-0 text-indigo-500" />
              <span className="font-medium tabular-nums text-slate-800 dark:text-slate-100">{format(now, 'MMM d, yyyy · HH:mm')}</span>
            </div>
          </div>
        </div>
      </div>

      <div className="border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 px-5 py-4 sm:px-6">
        <div className="flex flex-wrap items-end gap-3">
          <div className="mb-2 flex w-full items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 sm:mb-0 sm:w-auto">
            <Filter className="h-4 w-4 text-indigo-500" aria-hidden />
            Filters
          </div>
          {currentUser.role === 'Admin' ? (
            <label className="flex min-w-[160px] flex-1 flex-col gap-1 text-xs font-semibold text-slate-600 dark:text-slate-300 sm:max-w-[200px]">
              Role
              <select
                value={roleFilter}
                onChange={(e) => setRoleFilter(e.target.value)}
                className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/80 px-3 py-2 text-sm text-slate-800 dark:text-slate-100 outline-none focus:ring-2 focus:ring-indigo-100"
              >
          ...
          {currentUser.role === 'HR' ? (
            <label className="flex min-w-[160px] flex-1 flex-col gap-1 text-xs font-semibold text-slate-600 dark:text-slate-300 sm:max-w-[220px]">
              Role
              <select
                value={roleFilter}
                onChange={(e) => setRoleFilter(e.target.value)}
          ...
          <label className="flex min-w-[200px] flex-1 flex-col gap-1 text-xs font-semibold text-slate-600 dark:text-slate-300">
            <span className="inline-flex items-center gap-1.5">
              <Search className="h-3.5 w-3.5 text-slate-400 dark:text-slate-500" aria-hidden />
            ...
            <input
              ...
              className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/80 px-3 py-2 text-sm text-slate-800 dark:text-slate-100 outline-none placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-100"
            />
          </label>
        </div>
      </div>

      <div className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 px-5 py-3 sm:px-6">
        <div className="flex flex-wrap items-center gap-4 text-[11px] text-slate-600 dark:text-slate-300">
          <span className="font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Legend</span>
          <span className="inline-flex items-center gap-1.5">
          ...
          <span className="inline-flex items-center gap-1.5">
            <span className="inline-flex h-5 w-5 items-center justify-center rounded border border-slate-200 dark:border-slate-700 bg-slate-100 text-[9px] font-bold text-slate-700 dark:text-slate-200">
              A
            </span>
            Absent
          </span>
      ...
      <div className="overflow-x-auto">
        {period === 'today' && (
          <table className="w-full min-w-[520px] border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                <th className="px-5 py-3.5">Team member</th>
                <th className="px-5 py-3.5">{BRAND_ID_LABEL}</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                <tr>
                  <td colSpan={3} className="px-5 py-14 text-center text-sm text-slate-500 dark:text-slate-400">
                    Loading...
                  </td>
                </tr>
                ...
                visibleTodayRows.map((row, idx) => (
                  <tr
                    key={row.id}
                    className={cn('transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/90', idx % 2 === 1 ? 'bg-slate-50 dark:bg-slate-900/50' : 'bg-white dark:bg-slate-900')}
                  >
                    <td className="px-5 py-3.5">
                      <div className="font-semibold text-slate-900 dark:text-slate-50">{row.name}</div>
                    ...
                    <td className="px-5 py-3.5 font-mono text-xs text-slate-600 dark:text-slate-300">
                      {row.gdc_id
                        ? formatBrandEmployeeId(row.gdc_id)
                        : formatBrandEmployeeId(displayIdByUserId.get(String(row.id)))}
                    </td>
                    <td className="px-5 py-3.5">
        ...
        {period === '7d' && (
          <table className="w-full min-w-[900px] border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                <th className="sticky left-0 z-[1] min-w-[160px] bg-white dark:bg-slate-900 px-5 py-3.5 shadow-[4px_0_12px_-4px_rgba(15,23,42,0.08)]">
                  Team member
                </th>
                <th className="sticky left-[160px] z-[1] min-w-[100px] bg-white dark:bg-slate-900 px-3 py-3.5 shadow-[4px_0_12px_-4px_rgba(15,23,42,0.08)]">
                  ID
                </th>
                {Array.from({ length: 7 }).map((_, i) => (
                  <th key={`day-${i}`} className="px-1.5 py-3.5 text-center">
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500">Day {i + 1}</span>
                    </div>
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                <tr>
                  <td colSpan={9} className="px-5 py-14 text-center text-sm text-slate-500 dark:text-slate-400">
                    Loading...
                  </td>
                </tr>
                ...
                visibleGrid7.map((row, idx) => {
                  const rowBg = idx % 2 === 1 ? 'bg-slate-50 dark:bg-slate-900/80' : 'bg-white dark:bg-slate-900';
                  return (
                  <tr
                    key={row.id}
                    className={cn('group', rowBg, 'hover:bg-indigo-50/40 dark:hover:bg-slate-800/70')}
                  >
                    <td
                      className={cn(
                        'sticky left-0 z-[1] border-r border-slate-100 dark:border-slate-800 px-5 py-3 shadow-[4px_0_12px_-4px_rgba(15,23,42,0.06)] transition-colors group-hover:bg-indigo-50/40 dark:group-hover:bg-slate-800/70',
                        rowBg
                      )}
                    >
                      <div className="font-semibold text-slate-900 dark:text-slate-50">{row.name}</div>
                      <div className="text-[11px] font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{row.role}</div>
                    </td>
                    <td
                      className={cn(
                        'sticky left-[160px] z-[1] border-r border-slate-100 dark:border-slate-800 px-3 py-3 font-mono text-[11px] text-slate-600 dark:text-slate-300 shadow-[4px_0_12px_-4px_rgba(15,23,42,0.06)] transition-colors group-hover:bg-indigo-50/40 dark:group-hover:bg-slate-800/70',
                        rowBg
                      )}
                    >
                      {formatBrandEmployeeId(row.gdc_id, row.id)}
                    </td>
              ...
              {loading ? (
                ...
                visibleSummary30.map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 font-mono text-xs text-slate-600 dark:text-slate-300">{formatBrandEmployeeId(row.gdc_id, row.id)}</td>
                    ...
                    <td className="px-5 py-3.5 text-center tabular-nums font-semibold text-slate-700 dark:text-slate-200">{row.absent}</td>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 px-5 py-3 text-[11px] text-slate-500 dark:text-slate-400 sm:px-6">
        <span>
