  ...
  const toast = (
    ...
    >
      <button
        type="button"
        onClick={() => onDismiss?.()}
        className={`pointer-events-auto w-max max-w-[min(calc(100vw-1.5rem),22rem)] animate-in fade-in slide-in-from-top-2 duration-300 rounded-xl border border-slate-200 dark:border-slate-700/90 bg-white dark:bg-slate-900 text-left shadow-[0_8px_30px_rgb(15,23,42,0.12),0_2px_8px_rgb(15,23,42,0.06)] outline-none transition hover:shadow-[0_12px_36px_rgb(15,23,42,0.14)] focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2 ${
      ...
      >
        <p
          id="auth-alert-desc"
          className={`whitespace-normal text-balance font-medium leading-snug tracking-tight text-slate-700 dark:text-slate-200 ${compact ? 'text-[13px] sm:text-sm' : 'text-sm sm:text-[15px]'}`}
        >
          {message}
        </p>
